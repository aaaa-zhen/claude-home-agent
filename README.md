...
### 智能家居控制
- **空调**："把客厅空调调到 26 度"、"主卧开冷气"、"空调都关了"（支持客厅/主卧/书房三台）
- **灯光**："开客厅灯"、"关餐厅灯"（Zigbee 智能开关，区分氛围灯/主灯/餐厅灯）
- **音乐**：通过 HomePod mini 播放音乐，控制播放/暂停/音量
- **Apple TV**：基本播放控制
- **电动窗帘**：开/关/停（Broadlink 射频控制，接入 HA）
- **门窗传感器**：实时监测大门开关状态，离家后开门自动发安全提醒
- **到家自动开灯**：晚上到家自动打开客厅氛围灯（HA automation）
- 查询设备状态："空调开着吗"、"现在室温多少"
...
- **高铁**："广州南到珠海今天还有车吗" → 实时余票 + 时刻（通过 12306，国内网络中转）
- **快递价格查询**："从珠海寄 1kg 到北京多少钱" → 对比顺丰、中通、圆通等多家报价
- **顺丰下单**：查完价格直接下单寄顺丰（`sf_order.py`，对接顺丰开放平台）
- **导航路线**：支持驾车 / 步行 / 骑行 / 公交，自动判断出行方式

### AI 图片生成
- "帮我画一张……" → 调用 gpt-image-2 生成图片，直接发回微信
- 支持竖版 / 横版 / 方形，可选画质
